...
from functional.predict_functional import GRU_predict, prophet, create_features, XGB_predict
...
if page == "Data loading and preprocessing":
    ...
    if radio_btn == 'Select data from local database':
        dataset = ('BA_data','RTX_data','NOC_data', 'data_MW', 'data_weather')
    ...
    if agree1:
        dataset = ('BA_data','NOC_data','RTX_data','RTX_data', 'data_MW', 'data_weather')
        option = st.selectbox('Select dataset for prediction from database',dataset)
        DATA_URL =('./HISTORICAL_DATA/'+option+'.csv')
