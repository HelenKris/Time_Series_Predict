...
def prophet_select(data_pred,period):
    ...
    # r2_score = round(r2_score(test, y_pred),3)
...
"""
    ...
    # st.header('Model: XGBRegressor. Test period: '+str(period)+' days')
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=train.index, y=train['y'], name="True value",line_color='deepskyblue'))
    fig.add_trace(go.Scatter(x=test.index[:-period], y=test['y'], name="True value of test period",line_color='royalblue'))
...
# def GRU_predict(feature_df,period, num_epochs, batch_size):
#     FEATUREAS = list(feature_df.columns)
#     columns_to_delete = ['y', 'yhat']
#     FEATUREAS = [x for x in FEATUREAS if x not in columns_to_delete]
#     y_columns = ["y"]
...
#     y_hat = pd.Series(scaler_y.inverse_transform(model.predict(x[:], batch_size=batch_size)).squeeze())
...
# def GRU_select(feature_df,period, num_epochs, batch_size):
...
#     # score_RNN = round(r2_score(plot_GRU_forecast[-4*period:-period]['y'], plot_GRU_forecast[-4*period:-period]['RNN_prediction']))
